...
with open('server/src/services/mailService.js', 'r', encoding='utf-8') as f:
    content = f.read()

# Define the old block regex
pattern = re.compile(
    r'(export async function sendVaccinationReminderMail.*?const mailOptions = \{.*?)html: `.*?`(.*?\}[\s]*;[\s]*const info = await transport\.sendMail\(mailOptions\);)', 
    re.DOTALL
)
...
if not pattern.search(content):
    ...
    with open('server/src/services/mailService.js', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Replaced email template successfully!")
